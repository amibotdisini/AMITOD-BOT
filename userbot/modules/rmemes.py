...
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in (
        ...
        await event.edit("😎😎😎")
        await asyncio.sleep(1)
        await event.edit("Mau Gw Bunuh Gak? Woi Lu kontol 😎😎😎")
...
async def _(event):
    ...
    if input_str == "call":
        ...
        animation_chars = [
            ...
            "`telfon tersambung.`",
            "`Telegram: Anak mana ni anj,kenapa ya tod menelfon ku?`",
            f"`Me: Njir Slow Pak Gw `@{DEFAULTUSER},`Udah Lupa Kah? GakpapaLah Lupakan Aja Gw Ingin memberitahukan bahwa disini ada si paling sombong`",
            "`Sombong nya ngeselin`",
            "`Aduh pokok nya dia itu terlalu sombong la`",
            "`telfon privasi terhubung...`",
            "`Me: Hello Pak Pulici saya ingin melaporkan bahwa disini ada si jamet si paling sombong`",
            "`Pulici: Siapa ini?`",
            f"`Me: Saya Ni Si Paling Murni @{DEFAULTUSER} Goblok bgt pulici udah lupa gw sad kali gw ",
            "`Pulici: Oo iyah tenang biar saya urus si paling sombong itu`",
            "`Me: Asiappp Makasih pulici.`",
            "`Pulici: Iya sama sama,telfon saya lagi nanti kalo ada si paling sombong ya`",
...
@register(outgoing=True, pattern="^.sombong(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return
    ...
    if (x == 2):
        await event.edit("**Disini ada si jamet mau jelek jelekin gw**")
    if (x == 3):
    ...
    if (x == 61):
        await event.edit("**GARA GARA SI JAMET**")
    if (x == 62):
        await event.edit("**GUA JADI GEDEG**")
    if (x == 63):
        await event.edit("**JAMET SOMBONG**")
    if (x == 64):
        await event.edit("**LAEN KLI JANGAN MERASA DI ATAS TETAP LA RENDAH DIRI**")
    if (x == 65):
