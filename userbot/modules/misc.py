...
async def shutdown_bot(event):
    ...
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**✫AM͢͢͢ITOD-฿OT** telah di matikan ya tod!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(event, "**✫AM͢͢͢ITOD-฿OT Berhasil di matikan ya tod!**")
...
async def restart_bot(rstrt):
    if rstrt.sender_id in SUDO_USERS:
        return
    await edit_or_reply(rstrt, "**✫AM͢͢͢ITOD-฿OT Berhasil di Restart ya tod**")
    if BOTLOG_CHATID:
        await rstrt.client.send_message(
            BOTLOG_CHATID, "#RESTART \n" "**✫AM͢͢͢ITOD-฿OT Berhasil Di Restart ya tod**"
        )
...
async def repo_is_here(event):
    ...
    await xx.edit(
        f"**Haloo Anak Ngentot**, __Gua Disini__ ★ **✫AM͢͢͢ITOD-฿OT** ★\n\n"
...
async def okgoogle(img):
    ...
    if message and message.media:
        ...
        return await edit_or_reply(img, "**Woy Tod Balas ke Gambar Ya Tod**")
    if photo:
        xx = await edit_or_reply(img, "`Processing...`")
        try:
            image = Image.open(photo)
        except OSError:
            await xx.edit("**Awokwowk Gambar tidak di dukung**")
            return
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if guess and imgspage:
            ...
            await xx.edit("**Lu Jelek Jadi Gw Tidak dapat menemukan apa pun.**")
            return
...
async def send(event):
    if not event.is_reply:
        return await edit_or_reply(
            event, "**Woy Tod Mohon Balas Ke Pesan Yang Ingin Dikirim Ya Tod!**"
        )
    chat = event.pattern_match.group(1)
    xx = await edit_or_reply(event, "**Kerana Lu Jelek Pesan Ini Berhasil Dikirim Ya Tod**")
    try:
        chat = int(chat)
    except ValueError:
        pass
    ...
    await xx.edit(f"**Hi Tod Lu Berhasil Mengirim Pesan Ini Ke** `{chat.title}`")
...
CMD_HELP.update(
    {
        "restart": f"**Plugin : **`Restart ✫AM͢͢͢ITOD-฿OT`\
        \n\n  •  **Syntax :** `{cmd}restart`\
