...
async def bot_ver(event):
    ...
    if which("git") is not None:
        ...
        await event.edit(
            "Awokwowk sayang sekali anda tidak memiliki git, Anda Menjalankan Bot - 'v1.beta.4'!"
        )
...
async def pipcheck(pip):
    ...
    if pipmodule:
        ...
        if pipout:
            if len(pipout) > 4096:
                await pip.edit("`Woy Tod Output Terlalu Besar, Dikirim Sebagai File`")
...
@ram_cmd(pattern=r"(?:rlantodak|rbotmi)\s?(.)?")
async def amireallyalive(alive):
    user = await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    await alive.edit("`Liat Nih Tod Si Paling Keren Beria...`")
...
@ram_cmd(pattern=r"(?:ramitod|rami)\s?(.)?")
async def amireallyalive(alive):
    user = await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    output = (
...
async def amireallyalive(alive):
    user = await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    await alive.edit("`Woi Tod Liat Nih Lagi Perkenalan diri...🗿`")
    await asyncio.sleep(1)
    await alive.edit("⚡️")
    await asyncio.sleep(3)
    output = (
    ...
    if ALIVE_LOGO:
        try:
            ...
            await alive.edit(
                output + "\n\n *`Woy Tod Logo Yang Disediakan Tidak Valid."
                "\nHi Anak Ngentod Pastikan Tautan Yang Anda Gunakan Valid`"
            )
            await asyncio.sleep(100)
            await alive.delete()
    else:
...
async def amireallyaliveuser(username):
    ...
    if message != ".aliveu" and message[7:8] == " ":
        ...
        global DEFAULTUSER
        DEFAULTUSER = newuser
        output = "Oke Tod successfully changed user to " + newuser + "!"
...
async def amireallyalivereset(ureset):
    ...
    await ureset.edit("`" "Oke Tod successfully reset user for alive!" "`")
