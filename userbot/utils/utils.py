...
async def autobot():
    ...
    await bot.send_message(
        BOTLOG_CHATID, "**GUA LAGI BIKIN BOT ASISSTANT DI @BOTFATHER YA NGENTOD, SABAR DULU YA TOD, MAKANYA GANTENG KONTOL!!**"
    )
    who = await bot.get_me()
    name = who.first_name + " Assistant ✫AM͢͢͢ITOD-฿OT"
    if who.username:
        username = who.username + "_ubot"
    else:
        username = "amitod" + (str(who.id))[5:] + "bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot.send_message(bf, f"@{username}")
            await asyncio.sleep(1)
            await bot.send_file(bf, "userbot/utils/styles/amitod.jpg")
            await asyncio.sleep(3)
